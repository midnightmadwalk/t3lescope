...
const showHideMessage = document.getElementById("showHideMessage");
const cumulativeDollarAmountH3 = document.getElementById(
  "cumulativeDollarAmount"
);
const nightModeToggle = document.getElementById("nightModeToggle");
let tempWithoutPrice = 0;
...
function saveHideRowsPreference(hide) {
  localStorage.setItem("hideRowsPreference", hide);
}

// Function to toggle hide/show rows without a price
function toggleHideRowsWithoutPrice(hide) {
  const rows = document.querySelectorAll(".rowHolder");

  rows.forEach((row) => {
    const priceCell = row.querySelector("td:nth-child(6)"); // Assuming "Price" is the 6th column
...
// Initialize row visibility to show all rows initially
let tempPreference = localStorage.getItem("hideRowsPreference") === "true";
console.log(tempPreference + " = start setting");
let hideRowsPreference = false;
...
addressForm.addEventListener("submit", async function (e) {
  ...
  async function getPriceFromDexscreener(
    ...
    if (selectedType === "lpOnly") {
      hideRowsPreference = true;
    }
    // Check if the name is "PulseX LP" and skip the API call
    const tokenNameCell = document.querySelector(
      `#${CSS.escape(contractAddress)} > td:first-child`
    );
    ...
    statusMessage.textContent = `Processing row ${
      rowIndex + 1
    } out of ${relevantRowCount}`;
    ...
    try {
      ...
      const highestLiquidityPair = filteredPairs.reduce(
        (maxPair, currentPair) =>
          currentPair.liquidity.usd > maxPair.liquidity.usd
            ? currentPair
            : maxPair
      );
      ...
      if (priceCell) {
        ...
        totalCell.textContent = (
          priceUsd * balanceCell.textContent
        ...
        if (checkbox.checked) {
          cumulativeDollarAmount += parseFloat(
            totalCell.textContent.replace(/[^0-9.-]+/g, "")
          );
        }

        cumulativeDollarAmountH3.textContent = `Wallet Token Value: ${cumulativeDollarAmount.toLocaleString(
          "en-US",
          {
            style: "currency",
            currency: "USD",
    ...
    .then(async (tokenData) => {
      ...
      if (tokenData.result && tokenData.result.length > 0) {
        ...
        for (let index = 0; index < tokenData.result.length; index++) {
          ...
          if (
            (selectedType === "lpOnly" && token.name === "PulseX LP") ||
            ...
            const contractAddressText = document.createTextNode(
              `${token.contractAddress.slice(
                0,
                5
              )}....${token.contractAddress.slice(-5)}`
            );
            contractAddressCell.setAttribute(
              "data-full-contract-address",
            ...
            if (selectedType === "lpOnly") {
              statusMessage.textContent =
                "Currently unable to obtain accurate pricing data for LPs. The below is for your reference of LP held in the submitted wallet.";
            }
            console.log(
              "tempCounter = " +
                tempCounter +
                " & TokenData.result.length-1 =" +
                (tokenData.result.length - 1) +
                " & Index = " +
                index
            );
            if (
              (tempCounter >= tokenData.result.length - 1 &&
                index === tokenData.result.length - 1) ||
              (tempCounter >= tokenData.result.length - 1 &&
                tokenData.result[tokenData.result.length - 1].name ===
                  "PulseX LP")
            ) {
              let rowsWithPrice = 0;
              ...
              for (const [
                apiIndex,
                contractAddress,
              ] of tokensToCallApi.entries()) {
                await getPriceFromDexscreener(
                  contractAddress,
                  apiIndex,
                  relevantRowCount
                );
                ...
                if (apiIndex === tokensToCallApi.length - 1) {
                  ...
                  if (selectedType !== "lpOnly") {
                    ...
                    hideRowsPreference = tempPreference;
                    console.log(
                      "2 hideRows = " +
                        hideRowsPreference +
                        " and tempPref = " +
                    ...
                    if (hideRowsPreference) {
                      ...
                      toggleHideRowsWithoutPrice(true);
                      saveHideRowsPreference(true);
                    } else {
                      saveHideRowsPreference(false);
                    }
                  }
                }

                // Hide rows without a price after all API calls
                if (selectedType !== "lpOnly") {
                  tempWithoutPrice = apiIndex - rowsWithPrice + 1;
                  showHideMessage.textContent = `Missing pricing data for ${tempWithoutPrice} tokens. ${
                    hideRowsPreference ? "Show them?" : "Hide them?"
                  }`;
                  toggleHideRowsWithoutPrice(hideRowsPreference);
                }
              }
            }
          }
...
// Show/Hide rows button click event listener
showHideMessage.addEventListener("click", function () {
  hideRowsPreference = !hideRowsPreference; // Toggle the preference
  toggleHideRowsWithoutPrice(hideRowsPreference); // Hide or show rows based on user's preference
  saveHideRowsPreference(hideRowsPreference); // Save user's preference
  showHideMessage.textContent = hideRowsPreference
    ? `Missing pricing data for ${tempWithoutPrice} tokens. Show them?`
    : `Missing pricing data for ${tempWithoutPrice} tokens. Hide them?`; // Update button text
});
